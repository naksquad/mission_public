...
outlw_MR_filter =
{
	...
	_magTypes = _this # 0;
	_magAmmoCounts = _this # 1;
	_magAmmoCaps = _this # 2;
...
outlw_MR_onDrag =
{
	...
	outlw_MR_dragType = _this # 1;
	outlw_MR_dragCount = _this # 0;
...
outlw_MR_addSource =
{
	...
	if (outlw_MR_listDragging) then
	{
		...
		{
			outlw_MR_sourceType = _this # 2;
			outlw_MR_sourceCount = _this # 1;
...
outlw_MR_addTarget =
{
	...
	if (outlw_MR_listDragging) then
	{
		if ([_this select 2, _this select 1] call outlw_MR_magVerified) then
		{
			outlw_MR_targetType = _this # 2;
			outlw_MR_targetCount = _this # 1;
