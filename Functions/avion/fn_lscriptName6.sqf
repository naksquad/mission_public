...
if (_prayGod == 1) then
{
   switch (_TheGodPhase2) do {
      case "Save to loadout 1": {
         ...
         profileNamespace setVariable ["Gsavepylon1", _all_pylon];
         saveprofileNamespace;
         hint "Loadout 1 Saved."
      };
      case "Save to loadout 2": {
         _all_pylon = getPylonMagazines (vehicle player);
         profileNamespace setVariable ["Gsavepylon2", _all_pylon];
         saveprofileNamespace;
         hint "Loadout 2 Saved."
      };
      case "Save to loadout 3": {
         _all_pylon = getPylonMagazines (vehicle player);
         profileNamespace setVariable ["Gsavepylon3", _all_pylon];
         saveprofileNamespace;
...
else
{
   switch (_TheGodPhase2) do {
      case "Load loadout 1": {
         if !(isnil {profileNamespace getVariable "Gsavepylon1"}) then {
               
            _load1 = profileNamespace getVariable "Gsavepylon1";
         
            private _pylons = _load1;
            missionNamespace setVariable ["pilotGunnerah99", _load1];
      ...
      case "Load loadout 2": {
          if !(isnil {profileNamespace getVariable "Gsavepylon2"}) then {
               
            _load2 = profileNamespace getVariable "Gsavepylon2";
            missionNamespace setVariable ["pilotGunnerah99", _load2];
      ...
      case "Load loadout 3": {
          if !(isnil {profileNamespace getVariable "Gsavepylon3"}) then {
               
            _load3 = profileNamespace getVariable "Gsavepylon3";
            missionNamespace setVariable ["pilotGunnerah99", _load3];
