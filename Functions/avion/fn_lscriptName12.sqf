...
if (_prayGod == 1) then
{
   switch (_TheGodPhase2) do {
      case "Save to loadout 1": {
         ...
         profileNamespace setVariable ["Jsavepylon1", _all_pylon];
         saveprofileNamespace;
         hint "Loadout 1 Saved."
      };
      case "Save to loadout 2": {
         _all_pylon = getPylonMagazines (vehicle player);
         profileNamespace setVariable ["Jsavepylon2", _all_pylon];
         saveprofileNamespace;
         hint "Loadout 2 Saved."
      };
      case "Save to loadout 3": {
         _all_pylon = getPylonMagazines (vehicle player);
         profileNamespace setVariable ["Jsavepylon3", _all_pylon];
         saveprofileNamespace;
...
else
{
   switch (_TheGodPhase2) do {
      case "Load loadout 1": {
         if !(isnil {profileNamespace getVariable "Jsavepylon1"}) then {
               
            _load1 = profileNamespace getVariable "Jsavepylon1";
         missionNamespace setVariable ["pilotGunnervt1", _load1];
      ...
      case "Load loadout 2": {
          if !(isnil {profileNamespace getVariable "Jsavepylon2"}) then {
               
            _load2 = profileNamespace getVariable "Jsavepylon2";
         missionNamespace setVariable ["pilotGunnervt1", _load2];
      ...
      case "Load loadout 3": {
          if !(isnil {profileNamespace getVariable "Jsavepylon3"}) then {
               
            _load3 = profileNamespace getVariable "Jsavepylon3";
         missionNamespace setVariable ["pilotGunnervt1", _load3];
