...
switch (true) do
{
	case (windDir > 337.5 && windDir < 22.4): {
		levent = "Wind BRG : N";
	};
	case (windDir > 22.5 && windDir < 67.4): {
		levent = "Wind BRG : NE";
	};
	case (windDir > 67.5 && windDir < 112.4): {
		levent = "Wind BRG : E";
	};
	case (windDir > 112.5 && windDir < 157.4): {
		levent = "Wind BRG : SE";
	};
	case (windDir > 157.5 && windDir < 202.4): {
		levent = "Wind BRG : S";
	};
	case (windDir > 202.5 && windDir < 247.4): {
		levent = "Wind BRG : SW";
	};
	case (windDir > 247.5 && windDir < 292.4) : {
		levent = "Wind BRG : W";
	};
	case (windDir > 292.5 && windDir < 337.4): {
		levent = "Wind BRG : NW";
	};
	default {
		levent = "Wind BRG :";
...
switch (_lemonde) do {
	case "Altis": {
		...
		private _mapCenter = getmarkerpos "jet";
		private _ORBAT = [];
		private _markers = [];
		private _images = [ ];
		private _overcast = overcast;
		private _isNight = !((dayTime > 6) && (dayTime < 20));
		private _scale = 0.15;
		private _simul = true;
		private _chase = "Select HQ";
	...
	case "Malden": {
		...
		private _parentDisplay = [] call BIS_fnc_displayMission;
		private _mapCenter = getmarkerpos "respawn_west";
		private _ORBAT = [];
		private _markers = [];
		private _images = [ ];
		private _overcast = overcast;
	...
	case "Tanoa": {
		...
		private _parentDisplay = [] call BIS_fnc_displayMission;
		private _mapCenter = getmarkerpos "elite";
		private _ORBAT = [];
		private _markers = [];
		private _images = [ ];
		private _overcast = overcast;
