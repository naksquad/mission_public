...
[(_this select 0), ["<t color='#0f1199'>Drop Your Brother</t>", {
	_this spawn ROSE_fnc_ledetachement;
}, [], 1, false, true, "", "_this distance _target < 10"]] remoteExec ["addAction", -2];
...
[
	 ...
	 {
		[ (_this select 0), ( _this select 1)] spawn ROSE_fnc_carry3;
...
[(_this select 0), ["<t color='#95ff00'>PLAYER MENU</t>", {
	createDialog 'nastynak007';
}, [], 1, false, true, "", ""]] remoteExec ["addAction", -2];
[(_this select 1), ["<t color='#95ff00'>PLAYER MENU</t>", {
	createDialog 'nastynak007';
}, [], 1, false, true, "", ""]] remoteExec ["addAction", -2];

waitUntil {
	isNull attachedTo leblesser
};