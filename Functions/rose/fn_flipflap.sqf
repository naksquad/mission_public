private ["_vehicle", "_player", "_vehi_destroy", "_vehi_notFilp", "_vehicleNm", "_vehi_EngOn", "_vehi_Nflip", "_vectorfin", "_vectomap"];
_vehicle = _this # 0;
_player = _this # 1;
...
if (_vectomap > 0) exitWith {
	hint parseText _vehi_Nflip;
};

if (count (crew _vehicle) >= 1) exitWith {
	hint parseText _vehi_notFilp;
};
