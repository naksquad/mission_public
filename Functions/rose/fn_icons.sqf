...
_QS_ST_otherDisplays = TRUE;									// BOOL. TRUE to add Unit/Vehicle Icon support for UAV Terminal and Artillery Computer. Runs a separate script tot handle these displays. Only works if  _QS_ST_map_enableUnitIcons = TRUE;
...
_QS_fnc_iconText = {
	...
	private _grpOwner = name (leader group _v);
//			_vn = '[AI]';
//VileAce Changed to show group AI belongs to
			_vn = format ["%1's [AI]",_grpOwner];
		};
	};
			...
			if (!isNil {_v getVariable 'QS_ST_mapClickShowCrew'}) then {
				if (_v getVariable 'QS_ST_mapClickShowCrew') then {
					...
					_crewCount = (count (crew _v)) - 1;
					{
						...
						if (!(_showAINames)) then {
							if (!isPlayer _x) then {
								private _grpOwner = name (leader group _x);
//			_					_na = '[AI]';
//VileAce Changed to show group AI belongs to
								_na = format ["%1's [AI]",_grpOwner];
...
//				_unitName = '[AI]';
//VileAce Changed to show group AI belongs to
				_unitName = format ["%1's [AI]",_grpOwner];
