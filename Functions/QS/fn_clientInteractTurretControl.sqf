...
if (_v getVariable 'QS_turretL_locked') then {
	...
	player setUserActionText [(QS_turretActions select _actionIndex),((player actionParams (QS_turretActions select _actionIndex)) select 0),(format ["<t size='2'>%1</t>",((player actionParams (QS_turretActions select _actionIndex)) select 0)])];
};
