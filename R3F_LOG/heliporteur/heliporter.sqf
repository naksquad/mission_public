							...
							[_heliporteur, _objet] spawn
							{
								...
								while {_heliporteur getVariable "R3F_LOG_heliporte" == _objet} do
								{
									...
									if (!alive _heliporteur || (_a_ete_souleve && getPos _objet select 2 < 0)) exitWith
									{
										...
										if (typeof _heliporteur in _ghosthawk) then {
										//		[_heliporteur, ["<t color='#9990f9'>Ammo Drop</t>", "scripts\vehicle\drop\drop.sqf",[1],0,false,true,"", "driver _target == player"]] remoteExec ["addAction", 0, false];
											//	[( _this # 0),{_this addAction ["<t color='#9990f9'>Ammo Drop</t>", "scripts\vehicle\drop\drop.sqf",[1],0,false,true,"", "driver _target == player"]} ] remoteExec ["call", 0, true]; 
											};
